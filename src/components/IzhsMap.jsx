// src/components/IzhsMap.jsx
import { useEffect, useState, useRef } from "react";
import BookingModal from "./BookingModal";

export default function IzhsMap() {
  ...
  // 1) Инициализация карты и загрузка plots-izhs.json
  useEffect(() => {
    if (!window.ymaps) return;

    window.ymaps.ready(async () => {
      const el = document.getElementById("yandex-map-izhs");
      if (!el) return;
      el.innerHTML = "";
      ...
      try {
        const resp = await fetch("/plots-izhs.json");
        ...
        console.error("Ошибка загрузки plots-izhs.json:", e);
      }
    });
  }, []);

  // 2) Глобальный обработчик кликов по кнопке «Забронировать»
  useEffect(() => {
    const onDocClick = (e) => {
